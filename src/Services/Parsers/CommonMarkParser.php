...
class CommonMarkParser implements ParserInterface
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if ($ext['table'] ?? true) $environment->addExtension(new TableExtension());
        if ($ext['strikethrough'] ?? true) $environment->addExtension(new StrikethroughExtension());
        if ($ext['autolink'] ?? true) $environment->addExtension(new AutolinkExtension());
        if ($ext['task_lists'] ?? true) $environment->addExtension(new TaskListExtension());
